...
- Build 41 では mutual-follow DM の同期にも Supabase Auth user id を使います。`dm_threads.user_a_id` / `user_b_id` と `dm_messages.sender_user_id` は `auth.uid()` ベースで、Apple `providerUserId` はDM行に保存しません。
...
- Supabase Auth user id: remote actor id used for `users.id`, `profiles.user_id`, `follows.follower_user_id`, `blocks.blocker_user_id`, `reports.reporter_user_id`, `communities.creator_user_id`, `memberships.user_id`, `dm_threads.user_a_id`, `dm_threads.user_b_id`, and `dm_messages.sender_user_id`.

If there is no Supabase Auth session, follow/block/report/community/DM actions stay local and admin diagnostics record the fallback state. The app does not force login at launch and does not upload diary answers as part of social sync.
...
- Social and DM sync diagnostics also avoid tokens and private text. They may show sync status, counts, last error, and last sync time, but not access tokens, refresh tokens, Apple identity tokens, full Supabase keys, or DM message bodies.
