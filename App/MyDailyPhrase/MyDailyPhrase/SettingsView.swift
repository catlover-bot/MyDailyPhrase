        ...
        return AppSectionCard(
            ...
            if showsAuthTestDiagnostics {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    diagnosticRow(title: "Root AuthGate", value: authContext.rootAuthGateEnabled ? "enabled" : "disabled")
                    diagnosticRow(title: "Manual Auth Test", value: authContext.manualAuthTestEntryEnabled ? "enabled" : "disabled")
                    diagnosticRow(title: "Manual Apple Sign in", value: authContext.manualAppleSignInEnabled ? "enabled" : "disabled")
                    diagnosticRow(title: "現在の状態", value: authContext.currentAuthStateText)
                    diagnosticRow(title: "Provider", value: authContext.providerDisplayName)
                    diagnosticRow(title: "AuthUser.id", value: authContext.userID ?? "なし")
                    diagnosticRow(title: "Apple / Provider User ID", value: authContext.providerUserID ?? "なし")
                    diagnosticRow(title: "Display Name", value: authContext.displayName ?? "なし")
                    diagnosticRow(title: "Email", value: authContext.email ?? "なし")
                    diagnosticRow(title: "Roles", value: authContext.roleLabels.isEmpty ? "なし" : authContext.roleLabels.joined(separator: " / "))
                    diagnosticRow(title: "isAdmin", value: authContext.isAdmin ? "true" : "false")
                    diagnosticRow(title: "Capabilities", value: authContext.capabilityLabels.isEmpty ? "なし" : authContext.capabilityLabels.joined(separator: " / "))
    ...
    private var backendDiagnosticsSection: some View {
        ...
        return AppSectionCard(
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                diagnosticRow(title: "Supabase Auth", value: context.supabaseAuthStatus)
                diagnosticRow(title: "Supabase user id", value: context.supabaseUserID)
                diagnosticRow(title: "Access token present", value: context.supabaseAccessTokenPresent ? "true" : "false")
                diagnosticRow(title: "Refresh token present", value: context.supabaseRefreshTokenPresent ? "true" : "false")
                diagnosticRow(title: "Token expires", value: context.supabaseTokenExpiresAt)
                diagnosticRow(title: "Last Supabase Auth error", value: context.lastSupabaseAuthError)
                ...
                diagnosticRow(title: "Social sync", value: context.socialSyncStatus)
                diagnosticRow(title: "Social following", value: context.socialFollowingCount)
                diagnosticRow(title: "Social followers", value: context.socialFollowerCount)
                ...
                Text("401/403 の場合は Supabase Auth セッションがない、または auth.uid() と user_id が一致していない可能性があります。Supabase Auth連携、RLS policy、profiles テーブル名を確認してください。service_role はアプリに入れません。")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)

                Text("Follow / Block / Report の同期は Supabase Auth user id を使います。ローカルプレビュー相手や未ログイン時は local fallback が継続されます。")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
